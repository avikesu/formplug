...
function DesignerComponent({
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <p className={subTitleSizeClassMap[fontSize]}>{title}</p>
    </div>
  );
}
...
function FormComponent({
  ...
  return (
    <p
      className={`font-medium text-foreground/90 ${subTitleSizeClassMap[fontSize]}`}
    >
      {title}
    </p>
  );
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <p className="text-xs text-muted-foreground">
                Subtitle sizes stay below the heading sizes.
              </p>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex items-center justify-between rounded-xl border border-border/70 bg-muted/20 px-4 py-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Hide or show this subtitle block.
            </p>
          </div>
          <FormField
            control={form.control}
